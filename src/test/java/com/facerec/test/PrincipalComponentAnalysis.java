...
public class PrincipalComponentAnalysis {
    ...
    private void execute(String[] args) throws Exception {
    	BufferedImage bufferedImage = args.length >= 1 ? ImageIO.read(new File(args[0])) : ImageIO.read(new File("data/luoyafei.jpg"));
        ...
        try (Mat matrix = new OpenCVFrameConverter.ToMat().convert(new Java2DFrameConverter().convert(bufferedImage));
            ...
            System.out.println("Contour count " + contourCount);
    ...
    private void principalComponentAnalysis(Mat contour, int entry, Mat matrix) throws Exception {
        ...
        try (Mat data_pts = new Mat(contour.rows(), 2, CV_64FC1);
            ...
            ArrayList<Point2d> eigen_vecs = new ArrayList<Point2d>(2);
            ArrayList<Double> eigen_val = new ArrayList<Double>(2);
